...
// Web3.js contract connection (ENS-free)
...
// Alternative contract connection method to avoid ENS issues
export const GET_SIMPLE_CONTRACT = async () => {
  try {
    ...
    const contract = new ethers.Contract(HEALTH_CARE_ADDRESS, HEALTH_CARE_ABI, signer);
    return contract;
  } catch (error) {
    console.error("Simple contract connection error:", error);
    throw error;
  }
};
...
export const CHECK_PATIENT_REGISTERATION = async (_patientAddress) => {
  try {
    ...
    console.error("Patient registration check error:", error);
    throw error;
  }
};
